...
get_param_values <- function(mydir, para = NULL, vec, summary) {
  ...
  out <- data.frame(
    ...
    mfem = as.numeric(x[["pars"]][which(x[["pars"]][["Label"]] %in% c("NatM_uniform_Fem_GP_1", "NatM_p_1_Fem_GP_1", "NatM_break_1_Fem_GP_1")), 1:n]),
    ...
    mmale = as.numeric(x[["pars"]][which(x[["pars"]][["Label"]] %in% c("NatM_uniform_Mal_GP_1", "NatM_p_1_Mal_GP_1", "NatM_break_1_Mal_GP_1")), 1:n]),
